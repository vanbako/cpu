...
| `uart_tx_o` | debug UART output | `board_clk_i` | `implemented_same_domain_output` | Transmit-only UART output generated by the I25-S02 streamer in the current clock domain. |
...
- core, BRAM adapters, and UART status streamer use `.rst_n(core_rst_n)`;
- the status UART instance uses `.clk(board_clk_i)`;
- `uart_tx_o`, LED/status outputs, and debug projections are output-only paths from the current domain.
...
- Any future UART RX or loader input must add explicit synchronizers before it
  enters the SoC clock domain.
...
| --- | --- |
| Async inputs are documented. | Met by `board_reset_n_i`, `debug_halt_request_i`, and future UART RX open issues. |
| Reset synchronizers are documented and checked. | Met by `RESET_SYNC_STAGES`, `reset_sync_q`, and `core_rst_n` checks. |
| UART/debug crossings are audited. | Met by same-domain `uart_tx_o` and debug/status output items plus the raw halt open issue. |
